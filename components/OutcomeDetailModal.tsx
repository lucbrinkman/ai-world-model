...
export function OutcomeDetailModal({ isOpen, onClose, nodes }: OutcomeDetailModalProps) {
  ...
  const renderCategoryBar = (outcomes: Node[], color: string, label: string) => {
    const categoryTotal = outcomes.reduce((sum, n) => sum + n.p, 0);
    const heightPercent = grandTotal > 0 ? (categoryTotal / grandTotal) * 100 : 0;
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-800">
          <h2 className="text-2xl font-bold text-white">Outcome Distribution</h2>
          <button
            onClick={onClose}
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div className="flex flex-col">
              ...
              <div className="flex-1 bg-gray-800 border border-gray-700 rounded-lg overflow-hidden flex flex-col min-h-[400px]">
                {/* Good outcomes */}
                {renderCategoryBar(goodOutcomes, NODE_COLORS.GOOD.border, 'Good')}

                {/* Ambivalent outcomes */}
                {renderCategoryBar(ambivalentOutcomes, NODE_COLORS.AMBIVALENT.border, 'Ambivalent')}

                {/* Existential outcomes */}
                {renderCategoryBar(existentialOutcomes, NODE_COLORS.EXISTENTIAL.border, 'Existential')}
              </div>
            </div>

            {/* Right: Detailed List */}
            <div className="flex flex-col">
              <h3 className="text-lg font-semibold text-white mb-4">Detailed Probabilities</h3>
                  ...
                  <div>
                    ...
                    <div className="space-y-1">
                      {goodOutcomes.map(outcome => (
                        ...
                        >
                          <span className="text-gray-300 flex-1">{outcome.text}</span>
