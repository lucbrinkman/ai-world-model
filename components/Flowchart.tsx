...
export default function Flowchart({
  ...
  const [edgePreview, setEdgePreview] = useState<{
    ...
    node: NodeType | null;
    pos: { x: number; y: number } | null;
  }>({ edgeIndex: null, node: null, pos: null });
  ...
  const updateBounds = useCallback(
    (dragNodeIndex?: number, dragDeltaX?: number, dragDeltaY?: number) => {
      ...
      nodes.forEach((node, index) => {
        ...
        let y =
          (rect.top - containerRect.top) / zoomFactor - CONTAINER_BORDER_WIDTH;
        ...
        bounds.set(
          node.id,
          new DOMRect(x, y, rect.width / zoomFactor, rect.height / zoomFactor)
        );
      });
  ...
  useEffect(() => {
    ...
    const handleWheel = (e: WheelEvent) => {
      if (e.ctrlKey || e.metaKey) {
        ...
        const zoomFactor =
          1 + (delta > 0 ? ZOOM_SENSITIVITY : -ZOOM_SENSITIVITY);
        ...
        const canvasX =
          (container.scrollLeft + cursorX - CANVAS_PADDING) / (oldZoom / 100);
        const canvasY =
          (container.scrollTop + cursorY - CANVAS_PADDING) / (oldZoom / 100);

        // Calculate where that point should be after zoom to keep it under cursor
        const newScrollLeft =
          CANVAS_PADDING + canvasX * (newZoom / 100) - cursorX;
        const newScrollTop =
          CANVAS_PADDING + canvasY * (newZoom / 100) - cursorY;
  ...
  const handleNodeDragStateChangeWrapper = useCallback(
    (
      isDragging: boolean,
      shiftHeld: boolean,
      cursorX?: number,
  ...
  const handleBackgroundClick = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (
        lastInteractionRef.current.type === "editor-close" &&
  ...
  const handleAddNodeFromMenu = () => {
    ...
    const canvasX =
      (contextMenu.x - containerRect.left - CONTAINER_BORDER_WIDTH) /
      zoomFactor;
    const canvasY =
      (contextMenu.y - containerRect.top - CONTAINER_BORDER_WIDTH) / zoomFactor;
        ...
        >
          ...
          >
            ...
            {edges.map((edge, index) => {
              ...
              const targetNode =
                edge.target !== undefined ? nodes[edge.target] : undefined;
              ...
              const edgeGraphNode = graphData.nodes.find(
              ...
              const targetBounds = targetNode
                ? nodeBounds.get(targetNode.id)
                : undefined;

              // Coordinate conversion function for floating endpoints
              const screenToCanvasCoords = (
                screenX: number,
                screenY: number
              ) => {
                if (!containerRef.current || !scrollContainerRef.current)
              ...
              if (newArrowPreview && sourceNode.id === newArrowPreview.nodeId) {
                ...
                const newestNoEdge =
                  noEdgesFromNode[noEdgesFromNode.length - 1];
              ...
              const canDelete =
                ...
                onDeleteEdge &&
                sourceNode.type !== NT.START;
              const outgoingEdges = edges.filter(
                (e) => e.source === edge.source
              );
              const shouldShowDelete =
                canDelete &&
                (outgoingEdges.length === 1 || outgoingEdges.length === 2);
              ...
              return (
                <EdgeComponent
                  ...
                  onClick={
                    onEdgeClick ? () => handleEdgeClick(index) : undefined
                  }
                  onDelete={
                    shouldShowDelete ? () => onDeleteEdge(index) : undefined
                  }
                  onLabelUpdate={onEdgeLabelUpdate}
                  onEditorClose={handleEditorClose}
                  onDestinationDotHover={
                    onDestinationDotHover
                      ? () => onDestinationDotHover(index)
                      : undefined
                  }
                  onDestinationDotLeave={onDestinationDotLeave}
                  previewTargetNode={
                    edgePreview.edgeIndex === index ? edgePreview.node : null
                  }
                  previewTargetBounds={
                    edgePreview.edgeIndex === index && edgePreview.node
                      ? nodeBounds.get(edgePreview.node.id)
                      : undefined
                  }
                  previewFloatingPos={
                    isNewArrowPreview
                      ? newArrowPreviewPos
                      : edgePreview.edgeIndex === index
                        ? edgePreview.pos
                        : null
                  }
                />
          ...
          {nodes.map((node) => {
            ...
            const nodeSliderValue =
              graphNode?.probability !== null &&
              graphNode?.probability !== undefined
                ? graphNode.probability
                : undefined;
            ...
            return (
              <NodeComponent
                ...
                isProbabilityRootPreview={
                  previewProbabilityRootIndex !== null &&
                  previewProbabilityRootIndex !== undefined &&
                  node.index === previewProbabilityRootIndex
                }
                ...
                onSetProbabilityRoot={
                  onSetProbabilityRoot
                    ? () => onSetProbabilityRoot(node.index)
                    : undefined
                }
                onSetProbabilityRootHoverStart={
                  onSetProbabilityRootHoverStart
                    ? () => onSetProbabilityRootHoverStart(node.index)
                    : undefined
                }
                ...
                onSliderChange={
                  nodeSliderValue !== undefined && onSliderChange
                    ? (value) => onSliderChange(node.id, value)
                    : undefined
                }
                onSliderChangeComplete={
                  nodeSliderValue !== undefined && onSliderChangeComplete
                    ? () => onSliderChangeComplete(node.id)
                    : undefined
                }
          ...
          {(() => {
            ...
            return edges.map((edge, edgeIndex) => {
              ...
              const currentConnectionIndex =
                connectionIndexByNode.get(edge.source) || 0;
              connectionIndexByNode.set(
                edge.source,
              ...
              if (selectedEdgeIndex === edgeIndex) {
                ...
                hoveredDestinationDotIndex === edgeIndex ||
              ...
              const isPendingNewArrow =
                pendingNewArrow?.nodeId === sourceNode.id &&
                pendingNewArrow?.edgeIndex === currentConnectionIndex;
              ...
              return (
                <ConnectorDots
                  ...
                  onDestinationDotHover={
                    onDestinationDotHover
                      ? () => onDestinationDotHover(edgeIndex)
                      : undefined
                  }
                  onDestinationDotLeave={onDestinationDotLeave}
                  onDestinationDotDragStart={
                    onDestinationDotDragStart
                      ? () => onDestinationDotDragStart(edgeIndex)
                      : undefined
                  }
                  onDestinationDotDragEnd={onDestinationDotDragEnd}
                  screenToCanvasCoords={screenToCanvasCoords}
                  onPreviewChange={(node, pos) =>
                    handlePreviewChange(edgeIndex, node, pos)
                  }
                  onCreateNodeFromFloatingArrow={onCreateNodeFromFloatingArrow}
                  shouldStartDragging={isPendingNewArrow}
                  initialMousePos={
                    isPendingNewArrow ? pendingNewArrow?.mousePos : undefined
                  }
                />
              );
            });
