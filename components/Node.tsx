...
const Node = forwardRef<HTMLDivElement, NodeProps>(
  (
    ...
    const getBorderColor = () => {
      ...
      switch (type) {
        case NT.QUESTION:
          return isHovered || isNodeSelected
            ? NODE_COLORS.QUESTION.hover
            : NODE_COLORS.QUESTION.border;
        case NT.START:
        case NT.INTERMEDIATE:
          return NODE_COLORS.START.border;
        case NT.GOOD:
          return isHovered || isNodeSelected
            ? NODE_COLORS.GOOD.hover
            : NODE_COLORS.GOOD.border;
        case NT.AMBIVALENT:
          return isHovered || isNodeSelected
            ? NODE_COLORS.AMBIVALENT.hover
            : NODE_COLORS.AMBIVALENT.border;
        case NT.EXISTENTIAL:
          return isHovered || isNodeSelected
            ? NODE_COLORS.EXISTENTIAL.hover
    ...
    const getBackgroundColor = () => {
      ...
      switch (type) {
        case NT.QUESTION:
          return isHovered || isNodeSelected
            ? NODE_COLORS.QUESTION.bg + "4d"
            : NODE_COLORS.QUESTION.bg;
        case NT.START:
        case NT.INTERMEDIATE:
          return isHovered || isNodeSelected
            ? NODE_COLORS.START.bg + "4d"
            : NODE_COLORS.START.bg;
        case NT.GOOD:
          return isHovered || isNodeSelected
            ? NODE_COLORS.GOOD.hover
            : NODE_COLORS.GOOD.bg;
        case NT.AMBIVALENT:
          return isHovered || isNodeSelected
            ? NODE_COLORS.AMBIVALENT.hover
            : NODE_COLORS.AMBIVALENT.bg;
        case NT.EXISTENTIAL:
          return isHovered || isNodeSelected
            ? NODE_COLORS.EXISTENTIAL.hover
            : NODE_COLORS.EXISTENTIAL.bg;
        default:
          return "#000000";
      }
    };
    ...
    const getTextColor = () => {
      ...
      if ((isHovered || isNodeSelected) && type === NT.QUESTION)
        return NODE_COLORS.QUESTION.hover;
      return "#ffffff";
    };
    ...
    const handleMouseDown = (e: React.MouseEvent) => {
      ...
      if (internalRef.current) {
        ...
        internalRef.current.style.filter =
    ...
    const spacerWidth =
      numDigits * baseCharWidth + badgePadding * 2 + borderAdjustment; // +2px buffer
        ...
        {showSlider && (
          <div
            className={`absolute pointer-events-auto transition-opacity duration-200 ${
              isNodeSelected || isSliderDragging || isHovered
                ? "opacity-100"
        ...
        {showAddArrows &&
          ...
          (() => {
            ...
            return buttons.map(({ direction, icon: Icon, containerStyle }) => (
              ...
              >
                <Tooltip
                  content="Drag to add arrow"
                  position={direction === "bottom" ? "bottom" : "top"}
                >
                  <button
                    onMouseDown={(e) => {
                      e.stopPropagation();
