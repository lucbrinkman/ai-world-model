import { Node, Edge, NodeType, EdgeType, CANVAS_WIDTH, CANVAS_HEIGHT } from './types';

// Author's probability estimates (from original v4)
...
// Raw edge data from v4/graph.js
const EDGES_STR = `
...
function parseNodes(): Node[] {
  let nodeIndex = 0;
  const rawNodes = NODES_STR.split('\n').map(line => {
    const parts = line.trim().split(';');
    return {
      index: nodeIndex++,
      id: parts[1],
      type: parts[0] as NodeType,
      x: parseFloat(parts[2]) * XSCALE,
  ...
  return rawNodes.map(node => ({
    ...node,
    x: (node.x - minX) / (maxX - minX) * (CANVAS_WIDTH - 2 * CANVAS_PADDINGW) + CANVAS_PADDINGW,
    y: (node.y - minY) / (maxY - minY) * (CANVAS_HEIGHT - 2 * CANVAS_PADDINGH) + CANVAS_PADDINGH,
  }));
}

// Create id-to-index mapping
function createNodeIdMap(nodes: Node[]): Map<string, number> {
...
function parseEdges(nodeIdMap: Map<string, number>): Edge[] {
  return EDGES_STR.split('\n').map(line => {
    const parts = line.trim().split(';');
    const source = nodeIdMap.get(parts[1]);
    const target = nodeIdMap.get(parts[2]);

    if (source === undefined || target === undefined) {
      throw new Error(`Unknown node in edge: ${parts[1]} -> ${parts[2]}`);
    }

    return {
      yn: parts[0] as EdgeType,
      source,
      target,
      p: 0.0,
    };
...
// Export parsed data
export const nodes = parseNodes();
export const nodeIdMap = createNodeIdMap(nodes);
export const edges = parseEdges(nodeIdMap);
